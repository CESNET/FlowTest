...
TEST_CASE("timestamp gaps are generated correctly - single value and no max gap limit")
{
	const auto& timestamps = GenerateTimestamps(
		2,
		Timeval::From<TimeUnit::Seconds>(0),
		Timeval::From<TimeUnit::Seconds>(500));
	CHECK(timestamps.size() == 2);
	CHECK(timestamps.front() == Timeval::From<TimeUnit::Seconds>(0).ToNanoseconds());
	CHECK(timestamps.back() == Timeval::From<TimeUnit::Seconds>(500).ToNanoseconds());
...
TEST_CASE("timestamp gaps are generated correctly - basic case")
{
	const auto& timestamps = GenerateTimestamps(
		10,
		Timeval::From<TimeUnit::Seconds>(0),
		Timeval::From<TimeUnit::Seconds>(50),
		10);
	CHECK(timestamps.size() == 10);
	CHECK(timestamps.front() == Timeval::From<TimeUnit::Seconds>(0).ToNanoseconds());
	CHECK(timestamps.back() == Timeval::From<TimeUnit::Seconds>(50).ToNanoseconds());
...
TEST_CASE("timestamp gaps are generated correctly - basic case 2")
{
	const auto& timestamps = GenerateTimestamps(
		1000,
		Timeval::From<TimeUnit::Seconds>(10000),
		Timeval::From<TimeUnit::Seconds>(20000),
		20);
	CHECK(timestamps.size() == 1000);
	CHECK(timestamps.front() == Timeval::From<TimeUnit::Seconds>(10000).ToNanoseconds());
	CHECK(timestamps.back() == Timeval::From<TimeUnit::Seconds>(20000).ToNanoseconds());
...
TEST_CASE("timestamp gaps are generated correctly - extreme case")
{
	const auto& timestamps = GenerateTimestamps(
		11,
		Timeval::From<TimeUnit::Seconds>(0),
		Timeval::From<TimeUnit::Seconds>(10),
		1);
	CHECK(timestamps.size() == 11);
	CHECK(timestamps.front() == Timeval::From<TimeUnit::Seconds>(0).ToNanoseconds());
	CHECK(timestamps.back() == Timeval::From<TimeUnit::Seconds>(10).ToNanoseconds());
...
TEST_CASE("timestamp gaps are generated correctly - empty case")
{
	const auto& timestamps = GenerateTimestamps(
		0,
		Timeval::From<TimeUnit::Seconds>(0),
		Timeval::From<TimeUnit::Seconds>(100),
		1);
...
TEST_CASE("timestamp gaps are generated correctly - single packet")
{
	...
	CHECK(timestamps[0] == Timeval::From<TimeUnit::Seconds>(100).ToNanoseconds());
}
...
TEST_CASE("timestamp gaps are generated correctly - trim case")
{
	const auto& timestamps = GenerateTimestamps(
		...
		1);
	CHECK(timestamps.size() == 10);
	CHECK(timestamps.front() == Timeval::From<TimeUnit::Seconds>(10).ToNanoseconds());
	CHECK(timestamps.back() == Timeval::From<TimeUnit::Seconds>(19).ToNanoseconds());
...
TEST_CASE("timestamp gaps are generated correctly - trim case 2")
{
	const auto& timestamps = GenerateTimestamps(
		10,
		Timeval::From<TimeUnit::Seconds>(0),
		Timeval::From<TimeUnit::Seconds>(60),
		2);
	CHECK(timestamps.size() == 10);
	CHECK(timestamps.front() == Timeval::From<TimeUnit::Seconds>(0).ToNanoseconds());
	CHECK(timestamps.back() == Timeval::From<TimeUnit::Seconds>(18).ToNanoseconds());

...
public:
	PcppError() : std::runtime_error("Pcpp operation failed, see error log message") {}
};

/**
 * @brief A wrapper around pcpp::Packet that indicates failure by throwing an exception instead of returning false
 */
...
public:
	 ...
	 * @throw PcppError if an error occured (an appropriate error log message will be printed in such cases)
	 */
